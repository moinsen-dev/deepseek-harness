...
- [`@deepseek-ai/dsh-game-sim`](../../../../packages/game/game-sim/README.md) is the first provider: the `coin-chase` 3x3 deterministic simulation (four moves, three fixed coins, 10 points each, done when all are collected) and `gold-run`, a 5x5 grid whose two mines end the run — the larger mine-free search space makes a builder's first candidate usually miss the bar, so iteration visibly improves.
- [`@deepseek-ai/dsh-engine-godot`](../../../../packages/game/engine-godot/README.md) is the engine provider: each configured scenario runs once through Godot headless at load, and the validated deterministic NDJSON trace registers as a synchronous replay module — real engine outcomes behind the same seam, with live per-step engine execution deferred.
...
- [`@deepseek-ai/dsh-tool-game-build`](../../../../packages/game/tool-game-build/README.md) is the authoring half: `game_build` compiles a plain-JavaScript game source in a `node:vm` sandbox, validates the module contract, proves determinism by playing the probe inputs twice, and registers the game live; `game_remove` unloads builds, and rebuilding an id replaces the previous build. The loop is now complete: author → playtest → bar → rebuild.

The agent-side builder/critic iteration is deliberately not shipped as a new engine: it is composed from existing primitives (goal rounds, the Ralph loop, the workflow tool), and the example's drivers demonstrate three shapes — two scripted rounds (miss, then pass the bar), one builder/critic loop (three candidates, the third reaches the bar) followed by a model-driven `gauntlet_round`, and one Ralph loop whose two fresh children score `gold-run` gauntlet rounds until the bar passes (round 1 hits a mine, round 2 completes on a mine-free path), and one authoring run that builds a brand-new counter game with `game_build`, plays it with `game_play`, and reports the loop. The keyless snapshots pin all four assembled transcripts, and the real-model smoke verifies the scored tool result in the streamed session log rather than the model's claim.
